...
function! s:CIfDef(force)
 ...
 " #if .. #endif  nesting
  ...
  " #if 0 matching
  syn region ifdefUndefined  matchgroup=ifdefPreCondit4 start="^\s*#\s*if\s\+\(0\+\>\|(\s*0\+\>\)" matchgroup=ifdefPreCondit4 end="^\s*#\s*endif" contains=@ifdefClusterUndefined,ifdefElseInUndefinedToDefined
